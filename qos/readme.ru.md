...
`egress` это специальная под-очередь, куда перемещаются элементы согласно алгоритму приоретизации. Имеет два параметра:
* `EgressCapacity` - ёмкость, обязательный параметр.
* `EgressWorkers` - количество воркеров, которые перемещают элементы в egress из прочих под-очередей.
