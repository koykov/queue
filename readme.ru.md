...
В случае массовых ретраев, они могут синхронизироваться по времени и будут идти волнами, что будет мешать очереди их обработать.
Специально для этого случая есть параметр `Jitter`, который позволяет "размазать" ретраи во времени посредством добавления
к времени ретрая случайной задержки.

Параметр работает только с ненулевым `RetryInterval` и может работать совместно с `Backoff`.

Из коробки доступны jitter-ы:

* [Full](jitter/full.go) - возвращает случайное значение в интервале [0..RetryInterval).
* [Half](jitter/half.go) - возвращает случайное значение в интервале [RetryInterval/2 .. RetryInterval].
* [Decorrelated](jitter/decorrelated.go) - случайное значение между [Min..Max] с учётом предыдущего значения.
...
Вы можете написать реализацию `MetricsWriter`-а для иной TSDB, которую вы используете.
